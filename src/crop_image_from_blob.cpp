...
void crop_blob_to_image(cv::Mat big_image,cv::Rect rect , string file_contain_image, int *idx_file){
  ...
  cv::Mat mat_crop = big_image(rect); //crop in matrix

  cv::imwrite(file_contain_image +(char *)(*idx_file), mat_crop);
  *idx_file+=1;
