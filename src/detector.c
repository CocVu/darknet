                   ...
                   float hier_thresh, int dont_show, int ext_output, int save_labels)
{
  ...
  while(1){
    ...
    // not for opencv 3.4.1 not in gvc 
...
void write_txt_test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
                   float hier_thresh, int dont_show, int ext_output, int save_labels)
{
  list *options = read_data_cfg(datacfg);
  char *name_list = option_find_str(options, "names", "data/names.list");
  ...
  while(1){
    ...
    int destination_size = strlen(input);
    strcpy(output, input);
    for(int i =0 ; i< destination_size - 2; i++){
      output[i] = input[i];
    }

    output[destination_size -1] = 't';
    output[destination_size -2] = 'x';
    output[destination_size -3] = 't';
